...
def unsigncrypt_SCS1(params: GroupParams, alice_pub: int, bob: KeyPair,
    ...
    k = pow(base, (s * bob.x) % params.q, params.p)  # exponent reduced mod q (subgroup)
    k1, k2 = kdf_split(k)
...
def demo():
    ...
    raise NotImplementedError("Provide secure DSA-like subgroup params (p,q,g) to run demo.")
